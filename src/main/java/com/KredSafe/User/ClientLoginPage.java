...
public class ClientLoginPage extends TestBase {
	...
	public void RestoreValidation() throws InterruptedException, AWTException, TimeoutException {
		waitInSeconds(10);
	     driver.get("https://dev.kredsafe.net/user/education");
	     wait_for_page_load(10);
	     wait_for_element_present(Archive);
			wait_in_seconds(10);
			click_Element_Using_JS(Archive);
			wait_in_seconds(10);
			String errormessage = alertElement.getText();
			System.out.print("The error message is " + errormessage);
			if (errormessage.equals("No archived record found.")) {
				System.out.print("The actual error message for validation is " + errormessage);
				//AssertJUnit.assertEquals(alertElement, errormessage);
			}
	...
	public void Changepassword() throws InterruptedException, AWTException {
		...
		waitInSeconds(10);
		oldpw.sendKeys("Nilesh@2025");
		newpw.sendKeys("Nilesh@2030");
		newpwc.sendKeys("Nilesh@2030");
		waitInSeconds(10);
		pwsubmit.click();
		//click_Element_Using_JS(pwsubmit);
	}
	
	@FindBy(xpath = "//*[@id=\"facility_name\"]")
	...
	@FindBy(xpath = "//*[@id=\"template_ducument\"]")
	WebElement uplaodform;
	

	
	public void formrequest() throws InterruptedException, AWTException {
		 wait_for_page_load(10);	
			//click_Element_Using_JS(profile);
	       //wait_in_seconds(10);
	      driver.navigate().to("https://dev.kredsafe.net/user/es_forms/request-form");
	           
	      WebElement upload = driver.findElement(By.xpath("//*[@id=\"template_ducument\"]"));
			Actions act = new Actions(driver);
			act.moveToElement(upload).click().perform();
			wait_in_seconds(5);
	      ...
	      wait_in_seconds(5);
	      facilityname.sendKeys("Robert");
	      wait_in_seconds(5);

	      contactperson.sendKeys("Damc");
	      wait_in_seconds(5);

	      emailr.sendKeys("formtest@yopmail.com");
	      wait_in_seconds(5);

	      phoiner.sendKeys("1233217654");
	      wait_in_seconds(5);

	      Address.sendKeys("New Yors US");
	      wait_in_seconds(5);
	      scrollDownToElement(frsubmit);
	      frsubmit.click();
	      wait_in_seconds(5);
	      scrollDown();
	...
	public void Followup() throws InterruptedException, AWTException {
     ...
     // Subscription.click();
      ...
      scrollDown();
      scrollDown();
      //wait_for_element_present(bellicon);
    WebDriverWait wait = new WebDriverWait(driver, 10);
     WebElement bellIcon = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//i[@title='follow up']")));
     bellIcon.click();
      wait_in_seconds(5);
      folloupmsg.sendKeys("Reminder Message new");
      wait_in_seconds(5);
	...
	public void stepsToComplete1(String SSNNumber) throws InterruptedException { 
		wait_for_element_present(stepComplete);
		String stepcompletion = stepComplete.getText();
		System.out.println("The steps to complete on the Dashboard on login is " + stepcompletion);
		if (stepcompletion.equals("Congratulations! You have completed your on-boarding process.")) {
				...
				if (number >= 1) {
					for (int j = 0; j < number; j++) {
						...
						if (sectionName != null && sectionName.contains("Personal Details")) {
						    ...
						    System.out.println("Completing 'State Licenses or Certificates' step");
						   ...
						   // wait_for_element_present(submit);
						    ...
						    for (WebElement e : mandatoryFields) {
						        try {
						           // scrollDownToElement(e);
						            String redField = e.getText().trim();

						            switch (redField) {
						                case "Certification Name is required.":
						                    System.out.println("Filling Certification Name");
						                    wait_for_element_present(certification_name);
						                    certification_name.sendKeys("asd");
						                    break;

						                case "State Authority is required.":
						                    System.out.println("Selecting State Authority and entering date");
						                    Select selectStateAuthority = new Select(stateAuthority);
						                    selectStateAuthority.selectByValue("AL");
						                    certification_date.sendKeys("02/02/2025");
						                    wait_in_seconds(5);
						                ...
						                default:
						                    // You may log unhandled fields if needed
											break;
						            }
						        } catch (StaleElementReferenceException staleEx) {
						            System.out.println("Stale element encountered: " + staleEx.getMessage());
						            // Re-fetch elements if needed
						        }
						    }
						}

						else if (sectionName != null && sectionName.equals("Board Certification / Eligibility")) {
						    System.out.println("Completing 'Board Certification / Eligibility' step");
						  // wait_for_element_present(submit);
						    wait_in_seconds(5);
						   // scrollDownToElement(submit);
						     click_Element_Using_JS(submit);
						        ...
						        try {
						         ...
						         //   String redField = e.getText().trim();
						           // for (WebElement e : mandatoryFields) {
						            ...
						            switch (redField) {
						                case "Certificate Name is required.":
						                    System.out.println("Filling Certificate Name");
						                    wait_for_element_present(certification_name);
						                    certification_name.sendKeys("asd");
						                    break;

						                case "Authority is required.":
						                    System.out.println("Filling Authority");
						                    certificateAuthority.sendKeys("ASDF");
						                    break;

						                case "Date of Certification is required.":
						                    System.out.println("Filling Certification Date and saving");
						                    certification_date.sendKeys("02/02/2025");
						                    wait_in_seconds(5);
						                   click_Element_Using_JS(saveAndContinue1);
						                   wait_in_seconds(5);
						            ...
						            System.out.println("Stale element encountered: " + staleEx.getMessage());
						            // Optional: Re-fetch elements if needed
						        }
					...
					if (certCardTitle != null && certCardTitle.equals("State Licenses or Certificates")) {
					...
					} else if ((sectionName != null && sectionName.contains("Board Certification")) || 
					         (certCardTitle != null && certCardTitle.contains("Board Certification"))) {
					    System.out.println("Handling Board Certification / Eligibility section");
					    ...
					    for (WebElement e : mandatoryFields) {
					        ...
					        switch (redField) {
					            case "Certificate Name is required.":
					                System.out.println("Entering Certificate Name");
					                wait_for_element_present(certification_name);
					                certification_name.clear();
					                certification_name.sendKeys("Board Certified - Example");
					                break;

					            case "Authority is required.":
					                System.out.println("Entering Authority");
					                wait_for_element_present(certificateAuthority);
					                certificateAuthority.clear();
					                certificateAuthority.sendKeys("Medical Board Authority");
					                break;

					            case "Date of Certification is required.":
					                System.out.println("Entering Certification Date");
					                wait_for_element_present(certification_date);
					                certification_date.clear();
					                certification_date.sendKeys("02/02/2025");
					                break;

					            default:
					                System.out.println("Unrecognized required field: " + redField);
					                break;
					        }
					    }

					    scrollDownToElement(save);
					    waitInSeconds(2);
					    click_Element_Using_JS(save);
					    wait_for_page_load(10);
					    ...
					    try {
					        if (clickhere.isDisplayed()) {
					            wait_for_element_present(clickhere);
					            click_Element_Using_JS(clickhere);
					            System.out.println("Clicked 'Click Here' after saving");
					        }
					    } catch (NoSuchElementException e) {
					        System.out.println("ClickHere element not found.");
					    }
					}*/
